...
export default function QuizClient({ subjectId }: QuizClientProps) {
  ...
  useEffect(() => {
    const filteredQuestions = mockQuestions.filter(q => q.subjectId === subjectId);
  ...
  const handleAnswerSelect = (answerText: string) => {
    if (!isAnswerConfirmed) {
      ...
      setSelectedAnswers({...selectedAnswers, [questions[currentQuestion].id]: answerText});
    }
  };
  ...
  const finishQuiz = () => {
    if (!isQuizFinished) {
      ...
      router.push(`/results?subjectId=${subjectId}&answers=${encodedAnswers}&time=${quizDuration}`);
    }
  };
